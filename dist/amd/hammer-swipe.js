  ...
  var HammerSwipeCustomAttribute = (function () {
    function HammerSwipeCustomAttribute(element) {
      ...
      this.hammer = new _Hammer['default'].Manager(element, {
        recognizers: [[_Hammer['default'].Swipe, { direction: _Hammer['default'].DIRECTION_HORIZONTAL }]]
      });
      ...
      value: function handleSwipe(event) {
        if (this.callback) {
          ...
          switch (event.direction) {
            case _Hammer['default'].DIRECTION_LEFT:
              direction = 'left';
              break;
            case _Hammer['default'].DIRECTION_RIGHT:
              direction = 'right';
              break;
            default:
              direction = null;
